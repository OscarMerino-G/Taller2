    ...
    for (int i = 0; i < artistSongs.size() - 1; i++) {
        for (int j = i + 1; j < artistSongs.size(); j++) {
            if (artistSongs.at(i)->getNombre() > artistSongs.at(j)->getNombre()) {
                Song* temp = artistSongs.at(i);
                artistSongs.getList().removeAt(i);
                artistSongs.getList().insertAt(i, artistSongs.at(j - 1));
                artistSongs.getList().removeAt(j);
                artistSongs.getList().insertAt(j, temp);
            }
        }
    }
